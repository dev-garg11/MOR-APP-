...
export function StudentDashboardScreen({ onLogout }) {
  ...
  const loadStudentData = async () => {
    ...
    setError('');
    try {
      const [profRes, feeRes, attRes, assignRes, currRes] = await Promise.allSettled([
        getMyProfile(),
        getMyFees(),
        getMyAttendance(),
        getMyAssignments(),
      ...
      if (currRes.status === 'fulfilled' && currRes.value?.data) {
        setCurriculum(currRes.value.data);
      }
    } catch (err) {
      setError(err.message || 'Could not load your student dashboard. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmitAssignment = async () => {
    ...
    if (!submitContent.trim() && !submitFileUrl.trim()) {
      setSubmitError('Please provide submission notes or a project file URL.');
      return;
    }
    ...
    try {
      await submitMyAssignment(selectedAssignment.id, {
        content: submitContent.trim() || undefined,
        file_url: submitFileUrl.trim() || undefined,
      });
      setSubmitSuccess('🎉 Assignment submitted successfully to your faculty!');
  ...
  const handlePayOnline = async () => {
    if (!fees?.pending_amount || Number(fees.pending_amount) <= 0) {
      Alert.alert('Fees Cleared', 'You have no pending dues! Your tuition fees are completely cleared.');
      return;
    }

    setPaying(true);
    setPaymentStatusMsg('Initializing secure payment session…');

    try {
      const amountPaise = Math.round(Number(fees.pending_amount) * 100);
      const orderRes = await createRazorpayOrder(amountPaise, profile?.id);
      const orderData = orderRes.data || orderRes;

      if (!orderData?.order_id || !orderData?.key_id) {
        throw new Error('Could not generate Razorpay order. Check backend payment configuration.');
      }

      // Check if running on web browser
      const isScriptLoaded = await loadRazorpayScript();
      if (typeof window !== 'undefined' && window.Razorpay && isScriptLoaded) {
        const options = {
          key: orderData.key_id,
          amount: orderData.amount,
          currency: orderData.currency || 'INR',
          name: 'Morph Academy',
          description: `Fee Payment • Roll No: ${profile?.login_id || profile?.id}`,
          ...
          handler: async function (response) {
            setPaying(true);
            setPaymentStatusMsg('Verifying payment signature with server…');
            ...
            try {
              ...
              // Reload fresh fee balance and history from backend
              await loadStudentData();

              const verifyData = verifyRes.data || verifyRes;
              Alert.alert(
                '🎉 Payment Verified & Recorded!',
                `Amount: ₹${verifyData.amount || fees.pending_amount}\nPayment Ref: ${response.razorpay_payment_id}\n\nYour fee account has been updated successfully.`
              );
            } catch (verifyErr) {
              Alert.alert(
                '⚠️ Payment Verification Error',
                verifyErr.message || 'Payment received by gateway but server verification failed. Please contact admissions with your Payment ID.'
              );
            } finally {
              setPaying(false);
              setPaymentStatusMsg('');
        ...
        rzp.on('payment.failed', function (response) {
          ...
          Alert.alert(
            ...
            response.error?.description || 'Your payment was declined or cancelled. Your fee balance remains unchanged.'
          );
        });

        rzp.open();
      } else {
        // Fallback for standalone native app or when external script is restricted
        Alert.alert(
          '💳 Razorpay Checkout Ready',
          `Order ID: ${orderData.order_id}\nAmount: ₹${fees.pending_amount}\n\nPlease open this portal in browser to complete online payment, or pay at the campus admissions desk.`
        );
        setPaying(false);
        setPaymentStatusMsg('');
      }
    } catch (err) {
      Alert.alert('Payment Error', err.message || 'Could not initiate payment. Please try again later.');
      setPaying(false);
      setPaymentStatusMsg('');
    }
  };
